...
export default function Button({
  ...
  const iconNode = isLoading ? null : icon

  if (href) {
    return <Link href={href} className={classes}>{iconPosition === 'right' && iconNode}{children}{iconPosition === 'left' && iconNode}</Link>
  }
  return <button type={type} onClick={onClick} disabled={disabled || isLoading} className={classes}>{iconPosition === 'right' && iconNode}{children}{iconPosition === 'left' && iconNode}</button>
}
